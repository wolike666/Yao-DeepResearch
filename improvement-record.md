...
## 2026-06-07 Addendum: Token Usage Tracking
### Goal
- Add end-to-end token usage tracking for the research pipeline.
- Persist token stats into final result JSON.
- Print a concise token summary in the terminal.

### Changes
1. `src/my_deepresearch/llm_client.py`
- `LLMClient.chat(...)` now returns both `content` and `usage`.
- Added official usage extraction from OpenAI-compatible responses.
- Added fallback estimation when provider usage is unavailable.

2. `src/my_deepresearch/agent/state.py`
- Added `llm_calls`.
- Added aggregated `token_usage` with totals, by-purpose, by-step, and source counts.

3. `src/my_deepresearch/agent/utils.py`
- Added `record_llm_usage(...)`.
- Added `token_usage` and `llm_calls` to final result payload.

4. `src/my_deepresearch/agent/orchestrator.py`
- Tagged planner/extractor/synthesizer calls with usage context.
- Recorded per-step token usage into step logs.
- Fixed an edge case in `_dequeue_next_urls(...)` that could loop when URL candidates were insufficient.

5. `src/my_deepresearch/agent/reflector.py`
- Tagged reflector calls with usage context.

6. `main.py` and `scripts/run_single_research_windows.py`
- Added concise terminal token summary output.

### Validation
- `python -m compileall src`
- `python -m compileall main.py`
- `python -m compileall scripts/run_single_research_windows.py`
- Minimal stubbed run verified:
  - final result contains `token_usage`
  - final result contains `llm_calls`
  - `by_purpose` includes planner/extractor/reflector/synthesizer
  - `by_step` aggregates correctly
  - each step contains `token_usage`
