...
## 2026-06-07 补充记录：Token 消耗统计
### 目标
- 为整个 research 流程增加端到端的 token 消耗统计。
- 将 token 统计写入最终结果 JSON。
- 在终端打印简洁的 token 汇总信息。

### 改动内容
1. `src/my_deepresearch/llm_client.py`
- `LLMClient.chat(...)` 现在同时返回 `content` 和 `usage`。
- 增加对 OpenAI 兼容接口官方 `usage` 字段的提取。
- 当提供方拿不到 `usage` 时，增加降级估算逻辑。

2. `src/my_deepresearch/agent/state.py`
- 新增 `llm_calls`。
- 新增聚合后的 `token_usage`，包含总量、按用途、按 step、按来源统计。

3. `src/my_deepresearch/agent/utils.py`
- 新增 `record_llm_usage(...)`。
- 在最终结果 payload 中新增 `token_usage` 和 `llm_calls`。

4. `src/my_deepresearch/agent/orchestrator.py`
- 为 planner / extractor / synthesizer 调用增加 usage 上下文标签。
- 将每次调用的 token 信息按 step 写入日志并聚合到 state。
- 修复 `_dequeue_next_urls(...)` 在候选 URL 不足时可能出现循环的边界问题。

5. `src/my_deepresearch/agent/reflector.py`
- 为 reflector 调用增加 usage 上下文标签。

6. `main.py` 与 `scripts/run_single_research_windows.py`
- 增加终端 token 简洁汇总输出。
...
- 使用最小 stub 运行验证：
  - 最终结果包含 `token_usage`
  - 最终结果包含 `llm_calls`
  - `by_purpose` 包含 planner/extractor/reflector/synthesizer
  - `by_step` 聚合正确
  - 每个 step 都包含 `token_usage`
