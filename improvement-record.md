...
- ?? `record_llm_usage(...)`?
- ????? payload ??? `token_usage` ? `llm_calls`?

4. `src/my_deepresearch/agent/orchestrator.py`
- ? planner / extractor / synthesizer ???? usage ??????
- ?????? token ??? step ???????? state?
- ?? `_dequeue_next_urls(...)` ??? URL ???????????????

5. `src/my_deepresearch/agent/reflector.py`
- ? reflector ???? usage ??????

6. `main.py` ? `scripts/run_single_research_windows.py`
- ???? token ???????
...
- ???? stub ?????
  - ?????? `token_usage`
  - ?????? `llm_calls`
  - `by_purpose` ?? planner/extractor/reflector/synthesizer
  - `by_step` ????
  - ?? step ??? `token_usage`
